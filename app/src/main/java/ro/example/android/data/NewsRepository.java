...
public class NewsRepository {
    ...
    public Observable<List<ArticleEntity>> listenToAllArticles() {
    ...
    public Completable deleteAllArticles(){
        return database.articleDao().deleteArticles().subscribeOn(Schedulers.io());
    }

    public Completable fetchArticles(String query) {
        // First fetch the articles from the webservice
        return webService.fetchEverything(query)
                // ... then ...
    ...
    public Completable fetchTopNews() {
        // First fetch the articles from the webservice
        return webService.fetchTopHeadlines()
                // ... then ...
                .flatMapCompletable((List<ArticleDto> articleDtos) -> {
                    // Convert models from DTO to Entity (can also be done using a for)
                    List<ArticleEntity> articleEntities = articleDtos.stream()
